%% CS 435 Project 2 Finance inclass 2.3 (advanced)
%Ari Cooper
%Sep 15, 2016
...
% Read mixed data type into Matlab

fid = fopen('Google_price.dat.txt', 'r');

C_title = textscan(fid, '%s', 2);
C_data = textscan(fid, '%s %f');

Date = C_data{1};
price_close = C_data{2};
...
%% Find the date of the maximum daily return and minimum daily return over this time period 
% and verify your answer with online chart at Google ?nance.

[max_daily, max_index] = max(daily_ROR);
[min_daily, min_index] = min(daily_ROR);
